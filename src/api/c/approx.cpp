...
static inline af_array approx2(const af_array zi, const af_array xo,
                               ...
                               const float offGrid) {
    return getHandle(approx2<Ty>(getArray<Ty>(zi), getArray<Tp>(xo), xdim,
...
af_err af_approx2_uniform(af_array *zo, const af_array zi, const af_array xo,
    ...
    try {
        ...
        switch (zi_info.getType()) {
            case f32:
                output = approx2<float, float>(zi, xo, xdim, xi_beg, xi_step,
                                               yo, ydim, yi_beg, yi_step,
                                               method, offGrid);
                break;
            case f64:
                output = approx2<double, double>(zi, xo, xdim, xi_beg, xi_step,
                                                 yo, ydim, yi_beg, yi_step,
                                                 method, offGrid);
                break;
            case c32:
                output = approx2<cfloat, float>(zi, xo, xdim, xi_beg, xi_step,
                                                yo, ydim, yi_beg, yi_step,
                                                method, offGrid);
                break;
            case c64:
                output = approx2<cdouble, double>(zi, xo, xdim, xi_beg, xi_step,
                                                  yo, ydim, yi_beg, yi_step,
                                                  method, offGrid);
                break;
...
af_err af_approx2(af_array *zo, const af_array zi, const af_array xo,
                  ...
                  const float offGrid) {
    return af_approx2_uniform(zo, zi, xo, 0, 0.0, 1.0, yo, 1, 0.0, 1.0, method,
                              offGrid);
}
