...
af_err af_match_template(af_array *out, const af_array search_img, const af_array template_img, const af_match_type m_type)
{
    try {
        ...
        switch(sType) {
            ...
            case s64: output = match_template<intl  ,  float>(search_img, template_img, m_type); break;
            case u64: output = match_template<uintl ,  float>(search_img, template_img, m_type); break;
