...
af_err af_median_all(double *realVal, double *imagVal, const af_array in)
{
    try {
        ...
        switch(type) {
            ...
            case s64: *realVal = median<intl  >(in); break;
            case u64: *realVal = median<uintl >(in); break;
...
af_err af_median(af_array* out, const af_array in, const dim_t dim)
{
    try {
        ...
        switch(type) {
            ...
            case s64: output = median<intl  >(in, dim); break;
            case u64: output = median<uintl >(in, dim); break;
