...
static af_array var(const af_array& in, const bool isbiased, int dim) {
    ...
    Array<outType> divArr = createValueArray<outType>(
        oDims, scalar<outType>(isbiased ? iDims[dim] : iDims[dim] - 1));
    Array<outType> result =
        arithOp<outType, af_div_t>(redDiff, divArr, redDiff.dims());
...
static af_array var(const af_array& in, const af_array& weights, int dim) {
    ...
    Array<outType> meanArr =
        mean<outType, bType>(input, getArray<bType>(weights), dim);
    ...
    Array<outType> result =
        arithOp<outType, af_div_t>(accWDS, divArr, accWDS.dims());
