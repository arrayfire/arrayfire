...
MemoryManager::MemoryManager(int num_devices, unsigned max_buffers,
                                bool debug)
    : mem_step_size(1024)
...
void MemoryManager::removeMemoryManagement(int device) {
    ...
    // Do garbage collection for the device and leave the memory::memory_info struct
...
MemoryEventPair MemoryManager::alloc(const size_t bytes, bool user_lock) {
...
void MemoryManager::unlock(void *ptr, detail::Event &&e, bool user_unlock) {
...
void MemoryManager::userLock(const void *ptr) {
    ...
    if (iter != current.locked_map.end()) {
        ...
        memory::locked_info info = {false, true, 100};  // This number is not relevant
