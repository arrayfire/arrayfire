...
Array<Ty> approx1(const Array<Ty> &in, const Array<Tp> &pos,
    ...
    switch (method) {
        ...
        case AF_INTERP_LOWER:
            getQueue().enqueue(kernel::approx1<Ty, Tp, 1>, out, in, pos,
                               offGrid, method);
            break;
        case AF_INTERP_LINEAR:
        case AF_INTERP_LINEAR_COSINE:
            getQueue().enqueue(kernel::approx1<Ty, Tp, 2>, out, in, pos,
                               offGrid, method);
            break;
        case AF_INTERP_CUBIC:
        case AF_INTERP_CUBIC_SPLINE:
            getQueue().enqueue(kernel::approx1<Ty, Tp, 3>, out, in, pos,
                               offGrid, method);
            break;
        default: break;
    }
...
Array<Ty> approx2(const Array<Ty> &in, const Array<Tp> &pos0,
    ...
    switch (method) {
        case AF_INTERP_NEAREST:
        case AF_INTERP_LOWER:
            getQueue().enqueue(kernel::approx2<Ty, Tp, 1>, out, in, pos0, pos1,
                               offGrid, method);
            break;
        case AF_INTERP_LINEAR:
        ...
        case AF_INTERP_BILINEAR_COSINE:
            getQueue().enqueue(kernel::approx2<Ty, Tp, 2>, out, in, pos0, pos1,
                               offGrid, method);
            break;
        case AF_INTERP_CUBIC:
        ...
        case AF_INTERP_BICUBIC_SPLINE:
            getQueue().enqueue(kernel::approx2<Ty, Tp, 3>, out, in, pos0, pos1,
                               offGrid, method);
            break;
        default: break;
    }
...
#define INSTANTIATE(Ty, Tp)                                                \
    template Array<Ty> approx1<Ty, Tp>(                                    \
        const Array<Ty> &in, const Array<Tp> &pos,                         \
        const af_interp_type method, const float offGrid);                 \
    template Array<Ty> approx2<Ty, Tp>(                                    \
        const Array<Ty> &in, const Array<Tp> &pos0, const Array<Tp> &pos1, \
        const af_interp_type method, const float offGrid);
