...
void approx1(Param<InT> output, CParam<InT> input, CParam<LocT> xposition,
            ...
            for (dim_t idy = 0; idy < odims[1]; idy++) {
                ...
                for (dim_t idx = 0; idx < odims[0]; idx++) {
                    ...
                    if (x < 0 || idims[0] < x + 1) {
                        ...
                        interp(output, ooff + idx, input, ioff, x, method, 1,
                               clamp);
                    }
                }
            }
...
template<typename InT, typename LocT, int order>
void approx2(Param<InT> output, CParam<InT> input, CParam<LocT> xposition,
             CParam<LocT> yposition, float const offGrid,
        ...
        for (dim_t idz = 0; idz < odims[2]; idz++) {
            ...
            for (dim_t idy = 0; idy < odims[1]; idy++) {
                ...
                for (dim_t idx = 0; idx < odims[0]; idx++) {
                    ...
                    if (x < 0 || idims[0] < x + 1 || y < 0 ||
                        idims[1] < y + 1) {
                        out[ooff + idx] = scalar<InT>(offGrid);
                    } else {
                        interp(output, ooff + idx, input, ioffzw, x, y, method,
                               1, clamp);
                    }
                }
            }
