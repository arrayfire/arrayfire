...
void approx1(Param<Ty> yo, CParam<Ty> yi, CParam<Tp> xo, const int xdim,
    ...
    CUDA_LAUNCH((approx1_kernel<Ty, Tp, order>), blocks, threads, yo, yi, xo,
                xdim, xi_beg, xi_step, offGrid, blocksPerMat, batch, method);
    POST_LAUNCH_CHECK();
}
...
void approx2(Param<Ty> zo, CParam<Ty> zi, CParam<Tp> xo, const int xdim,
    ...
    CUDA_LAUNCH((approx2_kernel<Ty, Tp, order>), blocks, threads, zo, zi, xo,
                xdim, xi_beg, xi_step, yo, ydim, yi_beg, yi_step, offGrid,
                blocksPerMatX, blocksPerMatY, batch, method);
    POST_LAUNCH_CHECK();
}
}  // namespace kernel
}  // namespace cuda
