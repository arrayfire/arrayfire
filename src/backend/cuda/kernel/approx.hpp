...
__global__ void approx1_kernel(Param<Ty> out, CParam<Ty> in, CParam<Tp> xpos,
                               const float offGrid, const int blocksMatX,
                               const bool batch, af_interp_type method) {
...
template<typename Ty, typename Tp, int order>
__global__ void approx2_kernel(Param<Ty> out, CParam<Ty> in, CParam<Tp> xpos,
                               CParam<Tp> ypos, const float offGrid,
...
///////////////////////////////////////////////////////////////////////////
template<typename Ty, typename Tp, int order>
void approx1(Param<Ty> out, CParam<Ty> in, CParam<Tp> xpos, const float offGrid,
    ...
    CUDA_LAUNCH((approx1_kernel<Ty, Tp, order>), blocks, threads, out, in, xpos,
                offGrid, blocksPerMat, batch, method);
    POST_LAUNCH_CHECK();
}

template<typename Ty, typename Tp, int order>
void approx2(Param<Ty> out, CParam<Ty> in, CParam<Tp> xpos, CParam<Tp> ypos,
    ...
    CUDA_LAUNCH((approx2_kernel<Ty, Tp, order>), blocks, threads, out, in, xpos,
                ypos, offGrid, blocksPerMatX, blocksPerMatY, batch, method);
    POST_LAUNCH_CHECK();
}
}  // namespace kernel
}  // namespace cuda
