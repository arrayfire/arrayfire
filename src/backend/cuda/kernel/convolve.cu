...
__global__ void convolve2(Param<T> out, CParam<T> signal, int nBBS0, int nBBS1,
    ...
    const size_t C_SIZE =
        (THREADS_X + 2 * (fLen0 - 1)) * (THREADS_Y + 2 * (fLen1 - 1));
    ...
    int gy =
        THREADS_Y * ((blockIdx.y + blockIdx.z * gridDim.y) - b1 * nBBS1) + ly;
    ...
    switch (f0) {
        ...
        default: {
            if (f0 == f1) {
                switch (f1) {
                    case 6:
                        conv2Helper<T, aT, expand, 6, 6>(p, out, sig);
                        break;
                    case 7:
                        conv2Helper<T, aT, expand, 7, 7>(p, out, sig);
                        break;
                    case 8:
                        conv2Helper<T, aT, expand, 8, 8>(p, out, sig);
                        break;
                    case 9:
                        conv2Helper<T, aT, expand, 9, 9>(p, out, sig);
                        break;
                    case 10:
                        conv2Helper<T, aT, expand, 10, 10>(p, out, sig);
                        break;
                    case 11:
                        conv2Helper<T, aT, expand, 11, 11>(p, out, sig);
                        break;
                    case 12:
                        conv2Helper<T, aT, expand, 12, 12>(p, out, sig);
                        break;
                    case 13:
                        conv2Helper<T, aT, expand, 13, 13>(p, out, sig);
                        break;
                    case 14:
                        conv2Helper<T, aT, expand, 14, 14>(p, out, sig);
                        break;
                    case 15:
                        conv2Helper<T, aT, expand, 15, 15>(p, out, sig);
                        break;
                    case 16:
                        conv2Helper<T, aT, expand, 16, 16>(p, out, sig);
                        break;
                    case 17:
                        conv2Helper<T, aT, expand, 17, 17>(p, out, sig);
                        break;
                    default: {
                        char errMessage[256];
                        snprintf(errMessage, sizeof(errMessage),
                                 "\nCUDA 2D convolution doesn't support %dx%d "
                                 "kernel\n",
                                 f0, f1);
                        CUDA_NOT_SUPPORTED(errMessage);
                    };
            ...
            } else {
                {
                    char errMessage[256];
                    snprintf(errMessage, sizeof(errMessage),
                             "\nCUDA 2D convolution doesn't support "
                             "rectangular kernels\n");
                    CUDA_NOT_SUPPORTED(errMessage);
                };
            }
        ...
        for (int b2 = 0; b2 < filt.dims[2]; ++b2) {
            ...
            for (int b1 = 0; b1 < filt.dims[1]; ++b1) {
                ...
                CUDA_CHECK(cudaMemcpyToSymbolAsync(
                    kernel::cFilter, filt.ptr + (f1Off + f2Off + f3Off),
                    filterLen * sizeof(aT), 0, cudaMemcpyDeviceToDevice,
                ...
                CUDA_LAUNCH_SMEM((convolve1<T, aT, expand>), p.mBlocks,
                                 p.mThreads, p.mSharedSize, out, sig,
                                 filt.dims[0], p.mBlk_x, p.mBlk_y, p.o[0],
...
void convolve_2d(conv_kparam_t &p, Param<T> out, CParam<T> sig,
    ...
    for (int b3 = 0; b3 < filt.dims[3]; ++b3) {
        ...
        for (int b2 = 0; b2 < filt.dims[2]; ++b2) {
            ...
            // might cause issues
            CUDA_CHECK(cudaMemcpyToSymbolAsync(
                kernel::cFilter, filt.ptr + (f2Off + f3Off),
                filterLen * sizeof(aT), 0, cudaMemcpyDeviceToDevice,
                cuda::getActiveStream()));
...
void convolve_3d(conv_kparam_t &p, Param<T> out, CParam<T> sig,
    ...
    for (int b3 = 0; b3 < filt.dims[3]; ++b3) {
        ...
        // might cause issues
        CUDA_CHECK(cudaMemcpyToSymbolAsync(
            kernel::cFilter, filt.ptr + f3Off, filterLen * sizeof(aT), 0,
            cudaMemcpyDeviceToDevice, cuda::getActiveStream()));

        p.o[2] = (p.outHasNoOffset ? 0 : b3);
        p.s[2] = (p.inHasNoOffset ? 0 : b3);

        CUDA_LAUNCH_SMEM((convolve3<T, aT, expand>), p.mBlocks, p.mThreads,
                         p.mSharedSize, out, sig, filt.dims[0], filt.dims[1],
                         filt.dims[2], p.mBlk_x, p.o[2], p.s[2]);
...
#define INSTANTIATE(T, aT)                                                     \
    template void convolve_nd<T, aT, 1, true>(Param<T> out, CParam<T> signal,  \
                                              CParam<aT> filter,               \
                                              AF_BATCH_KIND kind);             \
    template void convolve_nd<T, aT, 1, false>(Param<T> out, CParam<T> signal, \
                                               CParam<aT> filter,              \
                                               AF_BATCH_KIND kind);            \
    template void convolve_nd<T, aT, 2, true>(Param<T> out, CParam<T> signal,  \
                                              CParam<aT> filter,               \
                                              AF_BATCH_KIND kind);             \
    template void convolve_nd<T, aT, 2, false>(Param<T> out, CParam<T> signal, \
                                               CParam<aT> filter,              \
                                               AF_BATCH_KIND kind);            \
    template void convolve_nd<T, aT, 3, true>(Param<T> out, CParam<T> signal,  \
                                              CParam<aT> filter,               \
                                              AF_BATCH_KIND kind);             \
    template void convolve_nd<T, aT, 3, false>(Param<T> out, CParam<T> signal, \
                                               CParam<aT> filter,              \
                                               AF_BATCH_KIND kind);
