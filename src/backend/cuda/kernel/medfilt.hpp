...
void medfilt2(Param<T> out, CParam<T> in, int w_len, int w_wid) {
    ...
    switch (w_len) {
        case 3:
            CUDA_LAUNCH((medfilt2<T, pad, 3, 3>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 5:
            CUDA_LAUNCH((medfilt2<T, pad, 5, 5>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 7:
            CUDA_LAUNCH((medfilt2<T, pad, 7, 7>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 9:
            CUDA_LAUNCH((medfilt2<T, pad, 9, 9>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 11:
            CUDA_LAUNCH((medfilt2<T, pad, 11, 11>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 13:
            CUDA_LAUNCH((medfilt2<T, pad, 13, 13>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
        case 15:
            CUDA_LAUNCH((medfilt2<T, pad, 15, 15>), blocks, threads, out, in,
                        blk_x, blk_y);
            break;
    }
...
void medfilt1(Param<T> out, CParam<T> in, int w_wid) {
    ...
    switch (w_wid) {
        case 3:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 3>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 5:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 4>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 7:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 5>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 9:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 6>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 11:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 7>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 13:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 8>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 15:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 9>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 17:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 10>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        case 19:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 11>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
        default:
            CUDA_LAUNCH_SMEM((medfilt1<T, pad, 62>), blocks, threads,
                             shrdMemBytes, out, in, w_wid, blk_x);
            break;
    }
