...
void morph(Param<T> out, CParam<T> in, int windLen) {
    ...
    switch (windLen) {
        case 2:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 2>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 3:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 3>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 4:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 4>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 5:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 5>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 6:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 6>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 7:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 7>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 8:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 8>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 9:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 9>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        case 10:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation, 10>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y);
            break;
        default:
            CUDA_LAUNCH_SMEM((morphKernel<T, isDilation>), blocks, threads,
                             shrdSize, out, in, blk_x, blk_y, windLen);
            break;
    }
...
void morph3d(Param<T> out, CParam<T> in, int windLen) {
    ...
    switch (windLen) {
        case 2:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 2>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        case 3:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 3>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        case 4:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 4>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        case 5:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 5>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        case 6:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 6>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        case 7:
            CUDA_LAUNCH_SMEM((morph3DKernel<T, isDilation, 7>), blocks, threads,
                             shrdSize, out, in, blk_x);
            break;
        default:
            CUDA_NOT_SUPPORTED(
                "Morph 3D does not support kernels larger than 7.");
    }
