        ...
        case BorderType::Symmetric:
            retVal =
                ((i < lb || i >= (lb + len)) ? ((len - 1) - ((i - lb) % len))
                                             : i - lb);
            break;
        case BorderType::ClampToEdge: retVal = clamp(i - lb, 0, len - 1); break;
        default:  // BorderType::Zero
            retVal = 0;
            break;
    }
    return retVal;
...
template<typename T, BorderType BType>
__global__
