...
void padBorders(Param<T> out, CParam<T> in, dim4 const lBoundPadding,
    ...
    switch (btype) {
        case AF_PAD_SYM:
            CUDA_LAUNCH((padBordersKernel<T, AF_PAD_SYM>), blocks, threads, out,
                        in, lBoundPadding[0], lBoundPadding[1],
                        lBoundPadding[2], lBoundPadding[3], blk_x, blk_y);
            break;
        case AF_PAD_CLAMP_TO_EDGE:
            CUDA_LAUNCH((padBordersKernel<T, AF_PAD_CLAMP_TO_EDGE>), blocks,
                        threads, out, in, lBoundPadding[0], lBoundPadding[1],
                        lBoundPadding[2], lBoundPadding[3], blk_x, blk_y);
            break;
        default:
            CUDA_LAUNCH((padBordersKernel<T, AF_PAD_ZERO>), blocks, threads,
                        out, in, lBoundPadding[0], lBoundPadding[1],
                        lBoundPadding[2], lBoundPadding[3], blk_x, blk_y);
            break;
    }
    POST_LAUNCH_CHECK();
