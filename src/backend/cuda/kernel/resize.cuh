...
template<typename T, InterpolationType method>
__global__
void resize(Param<T> out, CParam<T> in, const int b0,
    ...
    // core
    if (method == InterpolationType::Nearest) {
        resize_n(out, in, o_off, i_off, blockIdx_x, blockIdx_y, xf, yf);
    } else if (method == InterpolationType::Bilinear) {
        resize_b(out, in, o_off, i_off, blockIdx_x, blockIdx_y, xf, yf);
    } else if (method == InterpolationType::Lower) {
