...
static void scan_nonfinal_launcher(Param<To> out, Param<To> tmp,
    ...
    switch (threads_x) {
        case 32:
            CUDA_LAUNCH((scan_nonfinal_kernel<Ti, Tk, To, op, 32>), blocks,
                        threads, out, tmp, tflg, tlid, in, key, blocks_x,
                        blocks_y, lim, inclusive_scan);
            break;
        case 64:
            CUDA_LAUNCH((scan_nonfinal_kernel<Ti, Tk, To, op, 64>), blocks,
                        threads, out, tmp, tflg, tlid, in, key, blocks_x,
                        blocks_y, lim, inclusive_scan);
            break;
        case 128:
            CUDA_LAUNCH((scan_nonfinal_kernel<Ti, Tk, To, op, 128>), blocks,
                        threads, out, tmp, tflg, tlid, in, key, blocks_x,
                        blocks_y, lim, inclusive_scan);
            break;
        case 256:
            CUDA_LAUNCH((scan_nonfinal_kernel<Ti, Tk, To, op, 256>), blocks,
                        threads, out, tmp, tflg, tlid, in, key, blocks_x,
                        blocks_y, lim, inclusive_scan);
            break;
    }
...
static void scan_final_launcher(Param<To> out, CParam<Ti> in, CParam<Tk> key,
    ...
    switch (threads_x) {
        case 32:
            CUDA_LAUNCH((scan_final_kernel<Ti, Tk, To, op, 32>), blocks,
                        threads, out, in, key, blocks_x, blocks_y, lim,
                        calculateFlags, inclusive_scan);
            break;
        case 64:
            CUDA_LAUNCH((scan_final_kernel<Ti, Tk, To, op, 64>), blocks,
                        threads, out, in, key, blocks_x, blocks_y, lim,
                        calculateFlags, inclusive_scan);
            break;
        case 128:
            CUDA_LAUNCH((scan_final_kernel<Ti, Tk, To, op, 128>), blocks,
                        threads, out, in, key, blocks_x, blocks_y, lim,
                        calculateFlags, inclusive_scan);
            break;
        case 256:
            CUDA_LAUNCH((scan_final_kernel<Ti, Tk, To, op, 256>), blocks,
                        threads, out, in, key, blocks_x, blocks_y, lim,
                        calculateFlags, inclusive_scan);
            break;
    }
...
static void bcast_first_launcher(Param<To> out, Param<To> tmp, Param<int> tlid,
    ...
    dim3 blocks(blocks_x * out.dims[2], blocks_y * out.dims[3]);
    uint lim = divup(out.dims[0], (threads_x * blocks_x));
    CUDA_LAUNCH((bcast_first_kernel<To, op>), blocks, threads, out, tmp, tlid,
