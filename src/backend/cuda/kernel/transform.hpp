...
void transform(Param<T> out, CParam<T> in, CParam<float> tf, const bool inverse,
    ...
    CUDA_CHECK(cudaMemcpyToSymbolAsync(
        c_tmat, tf.ptr, nTfs2 * nTfs3 * tf_len * sizeof(float), 0,
    ...
    blocks.z *= max((nTfs2 / nImg2), 1) * max((nTfs3 / nImg3), 1);

    if (inverse) {
        CUDA_LAUNCH((transform_kernel<T, true, order>), blocks, threads, out,
                    in, nImg2, nImg3, nTfs2, nTfs3, batchImg2, blocksXPerImage,
                    blocksYPerImage, perspective, method);
    } else {
        CUDA_LAUNCH((transform_kernel<T, false, order>), blocks, threads, out,
                    in, nImg2, nImg3, nTfs2, nTfs3, batchImg2, blocksXPerImage,
                    blocksYPerImage, perspective, method);
    }
    POST_LAUNCH_CHECK();
