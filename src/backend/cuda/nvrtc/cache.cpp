...
Kernel buildKernel(const string& nameExpr, const string& jit_ker,
    ...
    if (isJIT) {
        ...
        static const char* headers[] = {
            sourceStrings[0].c_str(),
            sourceStrings[1].c_str(),
            sourceStrings[2].c_str(),
            sourceStrings[3].c_str(),
            sourceStrings[4].c_str(),
            sourceStrings[5].c_str(),
            sourceStrings[6].c_str(),
            sourceStrings[7].c_str(),
            sourceStrings[8].c_str(),
            sourceStrings[9].c_str(),
            sourceStrings[10].c_str()
        };
        NVRTC_CHECK(nvrtcCreateProgram(&prog, jit_ker.c_str(),
                    ker_name, NumHeaders, headers, includeNames));
    }
    ...
    if (!isJIT) {
        for (auto& s: opts) {
            compiler_options.push_back(&s[0]);
        }
    ...
    if (!isJIT) {
        NVRTC_CHECK(nvrtcGetLoweredName(prog, ker_name, &name));
    }
...
#define CASE_STMT(v) case v: retVal = #v; break
    switch(val) {
    CASE_STMT(af_add_t);
    CASE_STMT(af_sub_t);
    CASE_STMT(af_mul_t);
...
template<> string toString(bool val) {
    return string(val ? "true" : "false");
}

template<> string toString(af_op_t val) {
    return getOpEnumStr(val);
}

template<> string toString(const char* str) {
    return string(str);
}
