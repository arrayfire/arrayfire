...
af_err af_moddims(af_array *out, const af_array in, const unsigned ndims, const dim_type * const dims)
{
    ...
    dim_type nElems;
    af_err err = af_get_elements(&nElems,in);
    if (err!=AF_SUCCESS || nElems!=newDims.elements()) {
        return AF_ERR_SIZE;
    }
    ...
    try {
        ...
        switch(type) {
            ...
            case u32: output = moddims<unsigned int >(in,newDims); break;
            case u8:  output = moddims<unsigned char>(in,newDims); break;
