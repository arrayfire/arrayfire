...
public:
  conv2HelperCreateKernel(write_accessor<T> out, KParam oInfo,
                          ...
                          int sstep3, local_accessor<aT> localMem, const int f0,
                          const int f1, const bool expand) : out_(out), oInfo_(oInfo), signal_(signal), sInfo_(sInfo), impulse_(impulse), fInfo_(fInfo), nBBS0_(nBBS0), nBBS1_(nBBS1), ostep2_(ostep2), ostep3_(ostep3), sstep2_(sstep2), sstep3_(sstep3), localMem_(localMem), f0_(f0), f1_(f1), expand_(expand) {}
...
void conv2Helper(const conv_kparam_t<aT> &param, Param<T> out,
    ...
    getQueue().submit([&](auto &h) {
        ...
        h.parallel_for(sycl::nd_range{param.global, param.local},
                       conv2HelperCreateKernel<T, aT>(outAcc, out.info, signalAcc, signal.info, impulseAcc, filter.info, param.nBBS0, param.nBBS1, param.o[1], param.o[2], param.s[1], param.s[2], localMem, f0, f1, expand));
    });
}
