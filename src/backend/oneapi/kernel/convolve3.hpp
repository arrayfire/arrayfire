...
public:
 conv3HelperCreateKernel(write_accessor<T> out, KParam oInfo,
                         ...
                         local_accessor<aT> localMem, read_accessor<aT> impulse,
                         KParam fInfo, int nBBS0, int nBBS1, int ostep1, int ostep2,
                         int ostep3, int sstep1, int sstep2, int sstep3, const bool EXPAND) : out_(out), oInfo_(oInfo), signal_(signal), sInfo_(sInfo), localMem_(localMem), impulse_(impulse), fInfo_(fInfo), nBBS0_(nBBS0), nBBS1_(nBBS1), ostep1_(ostep1), ostep2_(ostep2), ostep3_(ostep3), sstep1_(sstep1), sstep2_(sstep2), sstep3_(sstep3), EXPAND_(EXPAND) {}
  void operator()(sycl::nd_item<3> it) const {
    ...
    if (gx < oInfo_.dims[0] && gy < oInfo_.dims[1] && gz < oInfo_.dims[2]) {
        ...
        dst[index(gx, gy, gz, oInfo_.strides[1], oInfo_.strides[2])] = (T)accum;
    }
  }
...
void conv3Helper(const conv_kparam_t<aT> &param, Param<T> &out,
    ...
    getQueue().submit([&](auto &h) {
        ...
        h.parallel_for(
            ...
            conv3HelperCreateKernel<T, aT>(outAcc, out.info, signalAcc, signal.info, localMem, impulseAcc, impulse.info, param.nBBS0, param.nBBS1, param.o[0], param.o[1], param.o[2], param.s[0], param.s[1], param.s[2], EXPAND));
    });
}
