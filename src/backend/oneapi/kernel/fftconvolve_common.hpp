...
// #include <kernel_headers/fftconvolve_pad.hpp>
...
void calcParamSizes(Param<T>& sig_tmp, Param<T>& filter_tmp, Param<convT>& packed,
                    Param<T>& sig, Param<T>& filter, const int rank,
                    AF_BATCH_KIND kind) {
    ...
    // Treat the incoming `packed` buffer as a string of real scalars
    // instead of complex numbers. SYCL is more strongly typed and
    // this mimics the originating OpenCL implementation while
    // adhering to SYCL's constraints
    // auto packed_num_elem = (*packed.data).get_range().size();
    // sycl::buffer<float> *poo = packed.data->template reinterpret<float>(sycl::range<1>{packed_num_elem * 2});
    // auto poo = (*packed.data).template reinterpret<float>(sycl::range<1>{packed_num_elem * 2});
    // sig_tmp.data =
