...
public:
    fftconvolve_multiplyCreateKernel(write_accessor<T> d_out, KParam oInfo, read_accessor<T> d_in1, KParam i1Info, read_accessor<T> d_in2, KParam i2Info, const int nelem, const int kind) : d_out_(d_out), oInfo_(oInfo), d_in1_(d_in1), i1Info_(i1Info), d_in2_(d_in2), i2Info_(i2Info), nelem_(nelem), kind_(kind) {}
    ...
    if (kind_ == AF_BATCH_NONE || kind_ == AF_BATCH_SAME) {
        ...
        const int ridx2 = ridx1 % (i2Info_.strides[3] * i2Info_.dims[3] * 2);
        const int iidx2 = iidx1 % (i2Info_.strides[3] * i2Info_.dims[3] * 2);
        ...
        // Treating complex output array as real-only array,
        // thus, multiply strides by 2
        const int ridx1 = ridx2 % (i1Info_.strides[3] * i1Info_.dims[3] * 2);
        const int iidx1 = iidx2 % (i1Info_.strides[3] * i1Info_.dims[3] * 2);
...
// #include "/home/gpryor/af-standalones/fftconvolve_pack/io.hpp"
// #include "/home/gpryor/af-standalones/fftconvolve_pack/msg.hpp"
...
#ifndef DONT_WRITE
    {
    OPEN_W("/home/gpryor/new-fftconvolve/fftconvolve_multiply_inputs.raw");
    WRITE(packed); WRITE(sig); WRITE(filter); WRITE(rank); WRITE(kind);
    }
#endif
...
//     }
    ...
    auto packed_tmp_buffer = (*packed.data).template reinterpret<typename convT::value_type>(sycl::range<1>{packed_num_elem * 2});
    auto sig_tmp_buffer = (*packed.data).template reinterpret<typename convT::value_type>(sycl::range<1>{packed_num_elem * 2});
    auto filter_tmp_buffer = (*packed.data).template reinterpret<typename convT::value_type>(sycl::range<1>{packed_num_elem * 2});

    getQueue().submit([&](auto &h) {
        sycl::accessor d_packed{packed_tmp_buffer, h, sycl::write_only, sycl::no_init};
        sycl::accessor d_sig_tmp{sig_tmp_buffer, h, sycl::read_only};
        sycl::accessor d_filter_tmp{filter_tmp_buffer, h, sycl::read_only};
        h.parallel_for(sycl::nd_range{global, local},
                       fftconvolve_multiplyCreateKernel<typename convT::value_type>(d_packed, packed.info, d_sig_tmp, sig_tmp.info, d_filter_tmp, filter_tmp.info, mul_elem, (int)kind));
    });
...
#ifndef DONT_WRITE
    {
      OPEN_W("/home/gpryor/new-fftconvolve/fftconvolve_multiply_outputs.raw");
      WRITE(packed);
    }
#endif
}
