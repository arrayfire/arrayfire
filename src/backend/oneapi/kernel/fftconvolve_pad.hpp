...
public:
    fftconvolve_padCreateKernel(write_accessor<outputType> d_out, KParam oInfo, read_accessor<inputType> d_in, KParam iInfo) : d_out_(d_out), oInfo_(oInfo), d_in_(d_in), iInfo_(iInfo) {}
...
void padDataHelper(Param<convT> packed, Param<T> sig, Param<T> filter, const int rank, AF_BATCH_KIND kind) {

#ifndef DONT_WRITE
    OPEN_W("/home/gpryor/new-fftconvolve/fftconvolve_pad.raw");
    ...
    getQueue().submit([&](auto &h) {
        sycl::accessor d_filter{*filter.data, h, sycl::read_only};
        sycl::accessor d_filter_tmp{filter_tmp_buffer, h, sycl::write_only, sycl::no_init};
        h.parallel_for(sycl::nd_range{global, local},
                       fftconvolve_padCreateKernel<T, typename convT::value_type>(d_filter_tmp, filter_tmp.info, d_filter, filter.info));
    });

    ONEAPI_DEBUG_FINISH(getQueue());

#ifndef DONT_WRITE
    WRITE(packed);  // final result
#endif
}
}  // namespace kernel
}  // namespace oneapi
