...
public:
  fftconvolve_reorderCreateKernel(write_accessor<T> d_out, KParam oInfo, read_accessor<T> d_in, KParam iInfo, KParam fInfo, const int half_di0, const int baseDim, const int fftScale, const bool EXPAND, const bool ROUND_OUT) : d_out_(d_out), oInfo_(oInfo), d_in_(d_in), iInfo_(iInfo), fInfo_(fInfo), half_di0_(half_di0), baseDim_(baseDim), fftScale_(fftScale), EXPAND_(EXPAND) , ROUND_OUT_(ROUND_OUT) {}
  void operator()(sycl::nd_item<1> it) const {
    ...
    if (ti0 < half_di0_) {
        ...
        int iidx2 = iInfo_.offset + ti3 + ti2 + ti1 + (ti0 - half_di0_) * 2 + 1;
        if (ROUND_OUT_)
          d_out_[oidx] = (T)round((d_in_[iidx1] + d_in_[iidx2]) / fftScale_);
        else
...
void reorderOutputHelper(Param<T> out, Param<convT> packed, Param<T> sig, Param<T> filter,
                         const int rank, AF_BATCH_KIND kind, bool expand) {
#ifndef DONT_WRITE
    OPEN_W("/home/gpryor/new-fftconvolve/fftconvolve_reorder_inputs.raw");
    WRITE(out); WRITE(packed); WRITE(sig); WRITE(filter); WRITE(rank); WRITE(kind); WRITE(expand);
#endif
...
//     }
    ...
    if (kind == AF_BATCH_RHS) {
      ...
      getQueue().submit([&](auto &h) {
          ...
          sycl::accessor d_out{*out.data, h, sycl::write_only, sycl::no_init};
          h.parallel_for(sycl::nd_range{global, local},
                         fftconvolve_reorderCreateKernel<T>(d_out, out.info, d_sig_tmp, sig_tmp.info, filter.info, sig_half_d0, rank, fftScale, expand, round_out));
                                                      // *out.data, out.info, *sig_tmp.data,  sig_tmp.info, filter.info, sig_half_d0, rank, fftScale);
      });
    }
