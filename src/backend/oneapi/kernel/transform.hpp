...
public:
  transformCreateKernel(write_accessor<T> d_out, const KParam out, read_accessor<T> d_in, const KParam in, read_accessor<float> c_tmat, const KParam tf, const int nImg2, const int nImg3, const int nTfs2, const int nTfs3, const int batchImg2, const int blocksXPerImage, const int blocksYPerImage, const af::interpType method, const bool INVERSE) : d_out_(d_out), out_(out), d_in_(d_in), in_(in), c_tmat_(c_tmat), tf_(tf), nImg2_(nImg2), nImg3_(nImg3), nTfs2_(nTfs2), nTfs3_(nTfs3), batchImg2_(batchImg2), blocksXPerImage_(blocksXPerImage), blocksYPerImage_(blocksYPerImage), method_(method) , INVERSE_(INVERSE) {}
...
#define MAX(A,B) (((A) > (B)) ? (A) : (B))
        int limages = MIN(MAX((int)(out_.dims[2] - imgId2 * nImg2_), 1), batchImg2_);
        ...
        int inoff =
          imgId2 * batchImg2_ * in_.strides[2] + imgId3 * in_.strides[3] + in_.offset;
        ...
        using TMatTy = typename std::conditional<PERSPECTIVE, float[9], float[6]>::type;
        TMatTy tmat;
...
#pragma unroll 3
        ...
        interp2(d_out_, out_, loco, d_in_, in_, inoff, xidi, yidi,
                0, 1, method_, limages, doclamp, 2);
}
...
template<typename T>
void transform(Param<T> out, const Param<T> in, const Param<float> tf, bool isInverse,
    ...
    int global_z = local[2] * fmax((nTfs2 / nImg2), 1) * fmax((nTfs3 / nImg3), 1);
...
#define INVOKE(PERSPECTIVE,INTERP_ORDER)                                \
    h.parallel_for(sycl::nd_range{global, local},                       \
                   ...
                   (d_out, out.info, d_in, in.info, d_tf, tf.info, nImg2, nImg3, \
                    nTfs2, nTfs3, batchImg2, blocksXPerImage, blocksYPerImage, \
                    method, isInverse));
