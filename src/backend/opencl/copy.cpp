...
Array<T> copyArray(const Array<T> &src) {
    ...
    if (src.isReady()) {
        kernel::memcopy<T>(*out.get(), out.strides(), *src.get(), src.dims(),
                           src.strides(), src.getOffset(), src.ndims());
    } else {
        Param info = {out.get(),
...
void multiply_inplace(Array<T> &src, double norm) {
    kernel::copy<T, T>(src, src, src.ndims(), scalar<T>(0), norm, true);
}
...
struct copyWrapper<T, T> {
    void operator()(Array<T> &dst, Array<T> const &src) {
        if (dst.isLinear() && src.isLinear() &&
            ...
            if (src.isReady()) {
                getQueue().enqueueCopyBuffer(
                    *src.get(), *dst.get(), src.getOffset() * sizeof(T),
                    dst.getOffset() * sizeof(T), src.elements() * sizeof(T),
                    nullptr, nullptr);
            } else {
                Param info = {dst.get(),
                              {{src.dims().dims[0], src.dims().dims[1],
            ...
            kernel::copy<T, T>(dst, src, src.ndims(), scalar<T>(0), 1.0, false);
        }
    }
};
