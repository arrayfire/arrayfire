...
static const char * _clblastGetResultString(clblast::StatusCode st)
{
    ...
    case clblast::StatusCode::kOpenCLBuildProgramFailure:  return "CL_BUILD_PROGRAM_FAILURE: OpenCL compilation error";
    ...
    case clblast::StatusCode::kInvalidLocalNumDimensions:  return "CL_INVALID_WORK_DIMENSION: Too many thread dimensions";
    case clblast::StatusCode::kInvalidLocalThreadsTotal:   return "CL_INVALID_WORK_GROUP_SIZE: Too many threads in total";
    case clblast::StatusCode::kInvalidLocalThreadsDim:     return "CL_INVALID_WORK_ITEM_SIZE: ... or for a specific dimension";
...
#define CLBLAST_CHECK(fn) do {                              \
        ...
        if (_clblast_st != clblast::StatusCode::kSuccess) { \
            ...
            snprintf(clblast_st_msg,                        \
                     sizeof(clblast_st_msg),                \
                     "CLBlast Error (%d): %s\n",            \
                     (int)(_clblast_st),                    \
                     _clblastGetResultString(               \
                         _clblast_st));                     \
                                                            \
            AF_ERROR(clblast_st_msg,                        \
                     AF_ERR_INTERNAL);                      \
        }                                                   \
    } while(0)
