...
void nonMaxSuppression(Param output, const Param magnitude, const Param dx, const Param dy)
{
    ...
    nonMaxOp(EnqueueArgs(getQueue(), global, threads),
             *output.data, output.info, *magnitude.data, magnitude.info,
             *dx.data, dx.info, *dy.data, dy.info, blk_x, blk_y);
...
void initEdgeOut(Param output, const Param strong, const Param weak)
{
    ...
    initOp(EnqueueArgs(getQueue(), global, threads),
           *output.data, output.info, *strong.data, strong.info, *weak.data, weak.info, blk_x, blk_y);
...
void suppressLeftOver(Param output)
{
    ...
    finalOp(EnqueueArgs(getQueue(), global, threads), *output.data, output.info, blk_x, blk_y);
...
void edgeTrackingHysteresis(Param output, const Param strong, const Param weak)
{
    ...
    auto edgeTraceOp = KernelFunctor<Buffer, const KParam,
                                     const unsigned, const unsigned, Buffer>(*entry.ker);
    ...
    while(notFinished) {
        notFinished = 0;
        getQueue().enqueueWriteBuffer(*d_continue, CL_TRUE, 0, sizeof(int), &notFinished);

        edgeTraceOp(EnqueueArgs(getQueue(), global, threads),
