...
template<typename T, af_op_t op>
void ireduceDim(Param out, cl::Buffer *oidx, Param in, int dim) {
...
template<typename T, af_op_t op>
void ireduceFirst(Param out, cl::Buffer *oidx, Param in) {
...
void ireduce(Param out, cl::Buffer *oidx, Param in, int dim) {
    if (dim == 0)
        return ireduceFirst<T, op>(out, oidx, in);
    else
        return ireduceDim<T, op>(out, oidx, in, dim);
}
