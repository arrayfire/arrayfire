...
kernel void reduce_all_kernel(global To *oData, KParam oInfo,
    ...
    const long tidx = get_local_id(0);
    const long tidy = get_local_id(1);
    const long tid  = tidy * DIMX + tidx;

    const long zid       = get_group_id(0) / groups_x;
    const long groupId_x = get_group_id(0) - (groups_x)*zid;
    const long xid       = groupId_x * get_local_size(0) * repeat + tidx;

    const long wid       = get_group_id(1) / groups_y;
    const long groupId_y = get_group_id(1) - (groups_y)*wid;
    const long yid       = groupId_y * get_local_size(1) + tidy;
    ...
    long last   = (xid + repeat * DIMX);
    long lim    = last > iInfo.dims[0] ? iInfo.dims[0] : last;

    To out_val = init;
    for (long id = xid; cond && id < lim; id += DIMX) {
    ...
    const long total_blocks = (get_num_groups(0) * get_num_groups(1) * get_num_groups(2));
    const long uubidx = (get_num_groups(0) * get_num_groups(1)) * get_group_id(2)
    ...
    if (total_blocks > 1) {
        ...
        if (tid == 0) {
            unsigned ticket = atomic_inc(retirementCount);
