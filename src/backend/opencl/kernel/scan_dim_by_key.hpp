...
namespace kernel
{
    ...
    static Kernel get_scan_dim_kernels(int kerIdx, int dim, bool calculateFlags, uint threads_y)
    {
        std::string ref_name =
            std::string("scan_") +
            std::to_string(dim) +
    ...
    static void scan_dim(Param &out, const Param &in, const Param &key, int dim)
    {
        try {
