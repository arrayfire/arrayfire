...
void sparseArithOpCSR(Param out, const Param values, const Param rowIdx,
    ...
    auto sparseArithCSROp =
        KernelFunctor<Buffer, const KParam, const Buffer, const Buffer,
                      const Buffer, const int, const Buffer, const KParam,
                      const int>(*entry.ker);

    NDRange local(TX, TY, 1);
    NDRange global(divup(out.info.dims[0], TY) * TX, TY, 1);

    sparseArithCSROp(EnqueueArgs(getQueue(), global, local), *out.data,
...
void sparseArithOpCOO(Param out, const Param values, const Param rowIdx,
    ...
    sparseArithCOOOp(EnqueueArgs(getQueue(), global, local), *out.data,
                     out.info, *values.data, *rowIdx.data, *colIdx.data,
                     values.info.dims[0], *rhs.data, rhs.info, reverse);
...
void sparseArithOpCSR(Param values, Param rowIdx, Param colIdx, const Param rhs,
    ...
    auto sparseArithCSROp =
        KernelFunctor<const Buffer, const Buffer, const Buffer, const int,
                      const Buffer, const KParam, const int>(*entry.ker);

    NDRange local(TX, TY, 1);
    NDRange global(divup(rhs.info.dims[0], TY) * TX, TY, 1);

    sparseArithCSROp(EnqueueArgs(getQueue(), global, local), *values.data,
...
void sparseArithOpCOO(Param values, Param rowIdx, Param colIdx, const Param rhs,
    ...
    auto sparseArithCOOOp =
        KernelFunctor<Buffer, Buffer, Buffer, const int, const Buffer,
                      const KParam, const int>(*entry.ker);

    NDRange local(THREADS, 1, 1);
    NDRange global(divup(values.info.dims[0], THREADS) * THREADS, 1, 1);

    sparseArithCOOOp(EnqueueArgs(getQueue(), global, local), *values.data,
                     *rowIdx.data, *colIdx.data, values.info.dims[0], *rhs.data,
                     rhs.info, reverse);
