...
\
        ...
        case AF_RANDOM_THREEFRY: kernel::uniformDistribution<T, AF_RANDOM_THREEFRY>(*out.get(), out.elements()*2, seed, counter); break;\
        }\
        return out;\
    }\
...
\
        switch(type) {\
        case AF_RANDOM_PHILOX: kernel::normalDistribution<TR, AF_RANDOM_PHILOX>(*out.get(), out.elements()*2, seed, counter); break;\
        case AF_RANDOM_THREEFRY: kernel::normalDistribution<T, AF_RANDOM_THREEFRY>(*out.get(), out.elements()*2, seed, counter); break;\
        }\
        return out;\
    }\
