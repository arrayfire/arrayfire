...
TEST(FFTConvolve2, CPP) {
    ...
    // false) however, notice that the 3rd dimension of filter is > 1. So, one to
    // many batch mode will be activated automatically where the 2d input signal
    // is convolved with each 2d filter and the result will written corresponding
    // slice in the output 3d array
    //![ex_image_convolve2]
...
TEST(FFTConvolve3, CPP) {
    ...
    // the filter, therefore many to many batch mode will be activated where each
    // 3d signal is convolved with the corresponding 3d filter
    //![ex_image_convolve3]
...
TEST(FFTConvolve, Docs_Unified_Wrapper) {
    ...
    // af_print(c);
    // c [10 1 1 1] = 0.3581 0.6777 1.0750 0.7679 0.5903 0.4851
    // 0.6598 1.2770 1.0734 0.8002
    //![ex_image_convolve_1d]
