...
protected:
    void SetUp() {
        backends_available = getAvailableBackends() & get<backend>(GetParam()).first;
        backends_available = backends_available && (getAvailableBackends() & get<backend>(GetParam()).second);
        ...
        if(backends_available) {
            ...
            values[0] =
                array(get<size>(GetParam()), get<type>(GetParam()));
...
std::string rngmatch_info(const ::testing::TestParamInfo<RNGMatch::ParamType> info) {
    ...
    ss << "size_" << get<size>(info.param)[0] << "_"
       ...
       << get<size>(info.param)[1] << "_"
       << get<size>(info.param)[2] << "_"
       << get<size>(info.param)[3] << "_seed_"
       << get<seed>(info.param) << "_type_"
       << get<type>(info.param);
    return ss.str();
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Combine(::testing::Values(AF_RANDOM_ENGINE_PHILOX),
